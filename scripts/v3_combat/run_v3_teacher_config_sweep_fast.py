...
import os
import pickle
from scripts.v3_combat import run_v3_teacher_config_sweep as sweep_base
import subprocess
import sys
import time
...
from scripts.v3_combat.run_v3_teacher_config_sweep import (
    PARAM_RANGES,
    PARAM_BUCKETS,
    _candidate_payload,
    _corner_sample,
...
def _summarize(results: list[dict[str, Any]], *, started: float, output_dir: Path) -> dict[str, Any]:
    from scripts.v3_combat.evaluate_v3_rollout_batch import _summarize as summarize_rollouts
...
def _first_combat_snapshot_fingerprint(args: argparse.Namespace) -> str:
    ...
    payload = {
        ...
        "files": [
            _file_fingerprint(root / "scripts/v3_combat/build_v3_first_combat_snapshots.py"),
...
def _prepare_first_combat_snapshots(args: argparse.Namespace, target_count: int) -> Path | None:
    ...
    command = [
        sys.executable,
        "-u",
        str(_repo_root() / "scripts/v3_combat/build_v3_first_combat_snapshots.py"),
        "--output-dir",
        str(snapshot_dir),
        "--seed-start",
...
def _fast_worker_init(
    ...
    from scripts.v3_combat import evaluate_v3_rollout_batch as eval_batch
