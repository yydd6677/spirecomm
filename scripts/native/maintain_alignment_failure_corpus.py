...
from scripts.native.compare_native_to_lightspeed_run import _attach_failure_metadata, _compare_seed_with_timeout, _summarize_clusters
...
def _dependency_cli_message(exc: BaseException) -> str | None:
    ...
    if "slaythespire is required for lightspeed/native comparison workflows" in text:
        return (
            "slaythespire is required for scripts/native/maintain_alignment_failure_corpus.py because it "
            "replays lightspeed/native comparison batches. Install the lightspeed Python "
            "package/build, or use native-only entrypoints such as scripts/native/run_native_run.py, "
            "scripts/native/run_native_sim.py, or scripts/native/export_model_run_checklist.py."
        )
    return None
