...
DEFAULT_OUT = Path("/home/yydd/下载/aliyun_search_budget_beam_node_sweep_v1")
...
SEARCH_BUDGET_GRID = (
    '{"beam_width":[24,28,32,40],'
    '"node_budget_per_root":[768,1024,1536,2048]}'
)

SEARCH_BUDGET_RANGES = '{"beam_width":[24,40],"node_budget_per_root":[768,2048]}'
...
OUT_DIR="$ROOT/teacher_sweep_runs/v3_teacher_beam_node_budget_sweep_v1"
...
exec "$PY" -u "$ROOT/scripts/v3_combat/run_v3_teacher_config_sweep_fast.py" \\
  ...
  --round0-count 300 \\
  --round1-count 100 --round1-stage-counts 100 --round1-stage-keeps 25 \\
  --round2-top 12 --round2-count 200 \\
  --round3-top 6 --round3-count 300 \\
...
log="$ROOT/logs/search_budget_beam_node.log"
echo "start search_budget_beam_node workers=$WORKERS log=$log"
(cd "$ROOT" && WORKERS="$WORKERS" nohup bash scripts/run_search_budget_beam_node.sh > "$log" 2>&1 & echo $! > logs/search_budget_beam_node.pid)
echo "started pid=$(cat "$ROOT/logs/search_budget_beam_node.pid")"
...
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
echo "== process =="
pgrep -af 'scripts/v3_combat/run_v3_teacher_config_sweep_fast.py.*v3_teacher_beam_node_budget_sweep_v1' || true
echo
echo "== latest log =="
if [[ -f "$ROOT/logs/search_budget_beam_node.log" ]]; then
  tail -60 "$ROOT/logs/search_budget_beam_node.log"
else
  echo "missing $ROOT/logs/search_budget_beam_node.log"
fi
echo
echo "== leaderboards =="
find "$ROOT/teacher_sweep_runs/v3_teacher_beam_node_budget_sweep_v1" -maxdepth 1 -name 'leaderboard_*.json' -printf '%TY-%Tm-%Td %TH:%TM %p\\n' 2>/dev/null | sort || true
"""
...
TMP_ROOT="${TMPDIR:-/tmp}/aliyun_search_budget_beam_node_smoke_$$"
rm -rf "$TMP_ROOT"
mkdir -p "$(dirname "$TMP_ROOT")"

export SPIRECOMM_TEACHER_SWEEP_PARAM_RANGES_JSON='{"beam_width":[14,16],"node_budget_per_root":[384,512]}'
export SPIRECOMM_TEACHER_SWEEP_ROUND1_GRID_JSON='{"beam_width":[14,16],"node_budget_per_root":[384,512]}'
...
"$PY" -u "$ROOT/scripts/v3_combat/run_v3_teacher_config_sweep_fast.py" \\
  ...
  --round0-count 1 \\
  --round1-count 1 --round1-stage-counts 1 --round1-stage-keeps 4 \\
  --round2-top 4 --round2-count 1 \\
...
if len(rows) != 4:
    raise SystemExit(f"smoke failed: expected 4 round1 rows, got {len(rows)}")
for row in rows:
    params = row.get("params") or {}
    if float(params.get("beam_width", 0)) < 24 or float(params.get("node_budget_per_root", 0)) < 768:
        raise SystemExit("smoke failed: candidate is below current default: " + repr(params))
errors = []
...
from spirecomm.ai.v3_combat_teacher import TeacherConfig, teacher_config_from_mapping

default = TeacherConfig()
assert default.beam_width == 24
assert default.node_budget_per_root == 768
profile = starting_profile("IRONCLAD")
cfg = teacher_config_from_mapping({"teacher_config": {"beam_width": 16, "node_budget_per_root": 512, "max_depth": 20}})
assert cfg.beam_width == 16
assert cfg.node_budget_per_root == 512
assert cfg.max_depth == 20
grid = json.loads('""" + SEARCH_BUDGET_GRID + """')
candidate_count = 1
for values in grid.values():
    candidate_count *= len(values)
for beam in grid["beam_width"]:
    assert beam >= default.beam_width
for node_budget in grid["node_budget_per_root"]:
    assert node_budget >= default.node_budget_per_root
print("bundle_ok", "starter_cards", len(profile.starter_deck_ids), "candidates", candidate_count)
"""


def readme() -> str:
    return """# Aliyun Beam + Node Budget Sweep Bundle

This bundle sweeps the v3 teacher combat search budget:

- `beam_width`
- `node_budget_per_root`

Current default is `(beam_width=24, node_budget_per_root=768, max_depth=20)`.
The sweep candidates are intentionally all larger than the current default; the default is still evaluated first as `round0_default`.

## Install

```bash
cd aliyun_search_budget_beam_node_sweep_v1
...
```bash
bash scripts/smoke_search_budget_beam_node_seed1.sh
```
...
```bash
WORKERS=$(nproc) bash scripts/run_search_budget_beam_node.sh
```

Background:

```bash
WORKERS=$(nproc) bash run_search_budget_beam_node_background.sh
...
- `round1`: 25 beam/node candidates, seed1-100, promote top12.
- `round2`: top12, seed1-200, promote top6.
- `round3`: top6, seed1-300, promote top3.
- `round4`: top3, seed1-600.

All proxy search overrides are disabled, so the candidate `beam_width` and `node_budget_per_root` values are not overwritten by the sweep scheduler.

Grid:

```json
{"beam_width":[24,28,32,40],"node_budget_per_root":[768,1024,1536,2048]}
```

Results are written under `teacher_sweep_runs/v3_teacher_beam_node_budget_sweep_v1/`.
"""
...
def build(out_dir: Path) -> Path:
    ...
    write_text(out_dir / "scripts" / "run_search_budget_beam_node.sh", run_search_budget_script(), executable=True)
    write_text(out_dir / "scripts" / "smoke_search_budget_beam_node_seed1.sh", smoke_script(), executable=True)
    write_text(out_dir / "scripts" / "status.sh", status_script(), executable=True)
    write_text(out_dir / "run_search_budget_beam_node_background.sh", run_background_script(), executable=True)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build Aliyun bundle for beam_width + node_budget_per_root sweep.")
