...
class StsRandom:
    ...
    The API intentionally mirrors the subset of ``random.Random`` that the
    native simulator already uses, so distribution work can migrate in layers.
    """
    ...
    def random(self, *args):
        """Python-compatible float random, plus lightspeed-like overloads.

        ``random()`` -> float in [0, 1)
        ``random(int_range)`` -> integer in [0, int_range], matching lightspeed
        ``random(start, end)`` -> integer inclusive for ints, float for floats
        """
        self.counter += 1
        if not args:
    ...
    # Compatibility with Python random.Random call sites.
