...
def run_stage_shared_prefix(
    *,
    ...
    cmd = [
        sys.executable,
        "-u",
        str(repo_root / "run_shared_prefix_sweep.py"),
        "--candidate-json",
        str(candidate_json),
        "--output-dir",
        str(shared_dir),
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Forwarded to evaluate_v3_rollout_batch.py; use 0 to disable combat no-HP-progress timeout.",
    )
    ...
    parser.add_argument(
        ...
        help="Evaluate each stage with run_shared_prefix_sweep.py so candidates share unchanged per-seed trajectory prefixes.",
    )
    parser.add_argument(
        "--racing",
        action="store_true",
