...
def _run(cmd: list[str], *, env: dict[str, str] | None = None, log_path: Path) -> None:
    ...
    with log_path.open("w", encoding="utf-8") as handle:
        ...
        handle.flush()
        result = subprocess.run(cmd, cwd=str(Path(__file__).resolve().parent), env=env, stdout=handle, stderr=subprocess.STDOUT)
...
def _shared_cmd(args: argparse.Namespace, shared_dir: Path) -> list[str]:
    cmd = [
        sys.executable,
        str(Path(__file__).resolve().parent / "run_shared_prefix_sweep.py"),
        "--repo-root",
        str(args.repo_root),
        "--candidate-json",
...
def _independent_cmd(args: argparse.Namespace, output_dir: Path, seeds: list[int]) -> list[str]:
    cmd = [
        sys.executable,
        str(Path(__file__).resolve().parent / "evaluate_v3_rollout_batch.py"),
        "--repo-root",
        str(args.repo_root),
        "--output-dir",
        str(output_dir),
